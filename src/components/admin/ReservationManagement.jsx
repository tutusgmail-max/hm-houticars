            ...
            <tbody>
              {paged.map((r) => (
                <tr key={r.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                  ...
                  <td className="p-4 font-medium">{reservationCustomerName(r)}</td>
                  <td className="p-4 text-white/50">{reservationCustomerPhone(r)}</td>
                  <td className="p-4">{r.car_name || '—'}</td>
                  <td className="p-4 text-white/50 text-xs whitespace-nowrap">
                    {reservationDateRange(r)}
                  </td>
                  <td className="p-4"><StatusBadge status={r.status} /></td>
                  <td className="p-4 text-right font-bold text-[#C9A84C]">{formatMoneyDh(r.total)}</td>
                  <td className="p-4">
