-- ============================================================================
--  HM HOUTI CARS — Migration Production FINALE
--  Fichier : MIGRATION_PRODUCTION_FINAL.sql
--  Version : Production v4.1 — Juin 2026
--  Instructions :
--    1. Ouvrir Supabase > SQL Editor > New query
--    2. Coller CE FICHIER ENTIER
--    3. Exécuter (Run)
--    4. Décommenter et exécuter l'UPDATE admin à la fin
...
-- ─── 1. FONCTION updated_at (partagée) ───────────────────────────────────────
...
-- Gestion colonne images (migration img → images)
DO $$
BEGIN
  IF EXISTS (
...
-- Storage policies: documents (bucket privé)
...
-- Storage policies: car images (bucket public)
...
CREATE POLICY "Public read car images"
  ON storage.objects FOR SELECT USING (bucket_id = 'image');
...
-- ─── 9. TRIGGER : notification admin sur nouvelle réservation ─────────────────
...
-- ─── 10. TRIGGER : overlap check (confirmed + completed bloquent seulement) ───
...
BEGIN
  ...
  ) THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.user_documents; END IF;
END;
$$;

-- ─── 12. RELOAD SCHEMA CACHE FINAL ───────────────────────────────────────────
NOTIFY pgrst, 'reload schema';

-- ─── 13. DÉFINIR VOTRE COMPTE ADMIN ──────────────────────────────────────────
-- Après votre premier signup sur le site, décommentez cette ligne
-- et remplacez par votre email :
...
-- ============================================================================
-- FIN MIGRATION — Projet HM Houti Cars prêt pour production
-- ============================================================================
