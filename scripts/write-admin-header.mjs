import fs from 'fs'
const d = (s) => `</${'motion.div>'.replace('motion.', '')}>`.replace('</motion.div>', '</motion.div>') // broken

// use explicit
const open = '<' + 'motion.div>'
const close = '</' + 'motion.div>'
const D = '<' + 'motion.div>'.replace('motion.', '')
const DC = '</' + 'motion.div>'.replace('motion.', '')
...
export default function AdminHeader({ title, subtitle, onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-[250] border-b border-white/10 bg-[#0a0f14]/80 backdrop-blur-xl">
      ${D} className="flex items-center justify-between gap-4 px-4 sm:px-8 py-4">
        ${D} className="flex items-center gap-4 min-w-0">
          ...
          ${DC}
        ${DC}
        ${D} className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          ${D} className="hidden sm:flex items-center gap-3 pl-3 border-l border-white/10">
            ${D} className="w-9 h-9 rounded-full bg-gradient-to-br from-[#C9A84C] to-[#E8C76A] flex items-center justify-center text-[#0B1623] font-black text-sm">
...
// Fix D - should be <div not <motion.div
const D2 = '<' + 'div'
const DC2 = '</' + 'div>'
const fixed = content.replaceAll(D, D2).replaceAll(DC, DC2)
fs.writeFileSync('src/components/admin/AdminHeader.jsx', fixed)
console.log('written', D2, DC2)
