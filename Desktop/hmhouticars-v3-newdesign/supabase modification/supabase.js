/**
 * lib/supabase.js — Supabase client + low-level helpers
 *
 * FIXES:
 *  1. Removed console.log that leaked the Supabase URL in production.
 *  2. createReservation: ref column may arrive duplicate — generate server-side
 *     uuid fallback so the insert never throws a unique-constraint violation on
 *     rapid double-submit.
 *  3. mapReservationToRow: total_price / total both set — some schema versions
 *     only have one. Now writes both safely.
 *  4. updateReservationStatus: added optimistic updated_at so dashboards
 *     sort correctly without waiting for a DB trigger.
 *  5. Removed booking.service.ts references to 'bookings' table (doesn't exist).
 *     The real table is 'reservations'.
 */
...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error(
    'Missing Supabase environment variables. Copy `.env.example` → `.env` and set ' +
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  ...
  global: {
    headers: { 'x-client-info': 'hmhouticars/3.0' },
  },
})
...
export async function signUp({ email, password, fullName, phone }) {
  ...
  if (data.user) {
    const { error: profileError } = await supabase.from('profiles').upsert(
      { id: data.user.id, full_name: fullName, phone, email, role: 'client' },
      { onConflict: 'id' },
    )
...
/** Detailed reservations for a car — status + reference for calendar legend. */
export async function getCarReservationsDetailed(carId) {
  const { data, error } = await supabase
    .from('reservations')
    .select('id, start_date, end_date, status, ref, customer_name')
    .eq('car_id', carId)
    .in('status', ['pending', 'confirmed'])
...
export function mapReservationToRow(reservation) {
  ...
  return {
    ...
    // FIX: write BOTH column aliases so either schema version works
    ref:             reservation.ref,
...
export async function createReservation(reservation) {
  // BUG FIX: Overlap check must run BEFORE insert.
  // Uses DB-level lte/gte to catch all date-range intersections.
  ...
  // FIX: Retry without columns the schema cache doesn't know yet (PGRST204).
  if (error?.code === 'PGRST204') {
    const {
      ref: _r, reference: _ref, total: _t, total_price: _tp,
      pickup_location: _p, return_location: _rl, documents: _d,
      cin_front_url: _cf, cin_back_url: _cb,
